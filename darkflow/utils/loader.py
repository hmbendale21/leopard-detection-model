    ...
    def load(self, path, src_layers):
        ...
        if walker.path is not None:
            if walker.offset != walker.size:
                print('Warning: expect {} bytes, found {}'.format(
...
class weights_walker(object):
    ...
    def __init__(self, path):
        ...
        if path is None: 
            ...
            # YOLOv2 can have 4 or 5 integers in the header
            # major, minor, revision, seen
            # If major*10+minor > 2, it usually has 5 ints
            major, minor = header[0], header[1]
            if (major * 10 + minor) >= 2:
                self.offset = 20
            else:
                self.offset = 16
    ...
    def walk(self, size):
        ...
        # Use np.fromfile or a more robust memmap approach
        with open(self.path, 'rb') as f:
            f.seek(self.offset)
            float32_1D_array = np.fromfile(f, dtype=np.float32, count=size)
